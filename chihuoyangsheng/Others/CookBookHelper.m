...
{
    DDXMLNode* child=[CookBookHelper getAttribute:node name:name];
    return child!=nil;
}

+(BOOL) hasValue:(DDXMLElement*) node name:(NSString*)name
{
    DDXMLElement* child=[CookBookHelper getNode:node name:name];
    if(child!=nil)
    {
        DDXMLNode* nilAttr=[CookBookHelper getAttribute:child name:@"nil" url:XSI];
        return nilAttr==nil;
    }
    return NO;
}
