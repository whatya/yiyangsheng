...
{
    DDXMLNode* child=[CustomerHelper getAttribute:node name:name];
    return child!=nil;
}

+(BOOL) hasValue:(DDXMLElement*) node name:(NSString*)name
{
    DDXMLElement* child=[CustomerHelper getNode:node name:name];
    if(child!=nil)
    {
        DDXMLNode* nilAttr=[CustomerHelper getAttribute:child name:@"nil" url:XSI];
        return nilAttr==nil;
    }
    return NO;
}
