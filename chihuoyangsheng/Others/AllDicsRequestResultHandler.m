...
#import "AllDicsSoapError.h"



@implementation AllDicsRequestResultHandler
...
if ((self=[super init])) {
        ...
        if(!classNames)
        {
            ...
            [AllDicsgetAllDictType class],@"http://service.dict.sys/^^getAllDictType",
            [AllDicsdictType class],@"http://service.dict.sys/^^dictType",
            nil]; 
...
{
    DDXMLNode* refAttr=[AllDicsHelper getAttribute:node name:@"Ref" url:MS_SERIALIZATION_NS];
    if(refAttr!=nil)
    {
        return [referencesTable objectForKey:[refAttr stringValue]];
    }
    
    DDXMLNode* nilAttr=[AllDicsHelper getAttribute:node name:@"nil" url:XSI];
    if(nilAttr!=nil && [[nilAttr stringValue]boolValue])
    {
        return nil;
    }

    DDXMLNode* typeAttr=[AllDicsHelper getAttribute:node name:@"type" url:XSI];
    if(typeAttr !=nil)
    {
    ...
    DDXMLNode* hrefAttr=[AllDicsHelper getAttribute:node name:@"href" url:@""];
    if(hrefAttr==nil)
    {
        hrefAttr=[AllDicsHelper getAttribute:node name:@"ref" url:@""];
    }
    if(hrefAttr!=nil)
    {
    ...
    DDXMLNode* idAttr=[AllDicsHelper getAttribute:node name:@"Id" url:MS_SERIALIZATION_NS];
    if(idAttr!=nil)
    {
...
-(id) createInstance:(Class) type node: (DDXMLNode*)node request :(AllDicsRequestResultHandler *)request
{
    SEL initSelector=@selector(initWithXml:__request:);
    id allocObj=[type alloc];
    IMP imp = [allocObj methodForSelector:initSelector];
    id (*func)(id, SEL, DDXMLNode*, AllDicsRequestResultHandler *) = (void *)imp;
...
-(DDXMLElement*) writeElement:(id)obj type:(Class)type name: (NSString*)name URI: (NSString*) URI parent:(DDXMLElement*) parent skipNullProperty:(BOOL)skipNullProperty
{
    ...
    if([obj conformsToProtocol:@protocol(AllDicsIReferenceObject)])
    {
...
-(void)setResponse:(NSData *)response
{
    ...
    OutputBody=[AllDicsHelper getNode:__root  name:@"Body" URI:@"http://schemas.xmlsoap.org/soap/envelope/"];
    OutputHeader=[AllDicsHelper getNode:__root  name:@"Header" URI:@"http://schemas.xmlsoap.org/soap/envelope/"];
    DDXMLElement* fault=[AllDicsHelper getNode:OutputBody  name:@"Fault" URI:@"http://schemas.xmlsoap.org/soap/envelope/"];
    if(fault!=nil)
    {
        DDXMLElement* faultString=[AllDicsHelper getNode:fault name:@"faultstring"];
        id faultObj=nil;
        DDXMLElement* faultDetail=[AllDicsHelper getNode:fault name:@"detail"];
        if(faultDetail!=nil)
        {
        ...
        OutputFault=[[AllDicsSoapError alloc] initWithDetails:[faultString stringValue] details:faultObj];
    }
}
...
-(void) sendImplementation:(NSMutableURLRequest*) request callbackDelegate:(AllDicsCLB) callbackDelegate
{
    if(self.EnableLogging) {
        NSString* strRequest = [[NSString alloc] initWithData: request.HTTPBody encoding: NSUTF8StringEncoding];
        NSLog(@"%@", strRequest);
