...
#import "CookBookfindCookbookByNameResponse.h"
...
@implementation CookBookRequestResultHandler
...
if ((self=[super init])) {
        ...
        if(!classNames)
        {
            ...
            [CookBookgetById class],@"http://service.cookbook.app/^^getById",
            [CookBookgetByIdResponse class],@"http://service.cookbook.app/^^getByIdResponse",
            [CookBookcookbook class],@"http://service.cookbook.app/^^cookbook",
            [CookBookfindCookbookByName class],@"http://service.cookbook.app/^^findCookbookByName",
            [CookBookpageParam class],@"http://service.cookbook.app/^^pageParam",
            [CookBookfindCookbookByNameResponse class],@"http://service.cookbook.app/^^findCookbookByNameResponse",
            [CookBookjsonData class],@"http://service.cookbook.app/^^jsonData",
            nil]; 
...
{
    DDXMLNode* refAttr=[CookBookHelper getAttribute:node name:@"Ref" url:MS_SERIALIZATION_NS];
    if(refAttr!=nil)
    {
        return [referencesTable objectForKey:[refAttr stringValue]];
    }
    
    DDXMLNode* nilAttr=[CookBookHelper getAttribute:node name:@"nil" url:XSI];
    if(nilAttr!=nil && [[nilAttr stringValue]boolValue])
    {
        return nil;
    }

    DDXMLNode* typeAttr=[CookBookHelper getAttribute:node name:@"type" url:XSI];
    if(typeAttr !=nil)
    {
    ...
    DDXMLNode* hrefAttr=[CookBookHelper getAttribute:node name:@"href" url:@""];
    if(hrefAttr==nil)
    {
        hrefAttr=[CookBookHelper getAttribute:node name:@"ref" url:@""];
    }
    if(hrefAttr!=nil)
    {
    ...
    DDXMLNode* idAttr=[CookBookHelper getAttribute:node name:@"Id" url:MS_SERIALIZATION_NS];
    if(idAttr!=nil)
    {
...
-(id) createInstance:(Class) type node: (DDXMLNode*)node request :(CookBookRequestResultHandler *)request
{
    SEL initSelector=@selector(initWithXml:__request:);
    id allocObj=[type alloc];
    IMP imp = [allocObj methodForSelector:initSelector];
    id (*func)(id, SEL, DDXMLNode*, CookBookRequestResultHandler *) = (void *)imp;
...
-(DDXMLElement*) writeElement:(id)obj type:(Class)type name: (NSString*)name URI: (NSString*) URI parent:(DDXMLElement*) parent skipNullProperty:(BOOL)skipNullProperty
{
    ...
    if([obj conformsToProtocol:@protocol(CookBookIReferenceObject)])
    {
...
-(void)setResponse:(NSData *)response
{
    ...
    OutputBody=[CookBookHelper getNode:__root  name:@"Body" URI:@"http://schemas.xmlsoap.org/soap/envelope/"];
    OutputHeader=[CookBookHelper getNode:__root  name:@"Header" URI:@"http://schemas.xmlsoap.org/soap/envelope/"];
    DDXMLElement* fault=[CookBookHelper getNode:OutputBody  name:@"Fault" URI:@"http://schemas.xmlsoap.org/soap/envelope/"];
    if(fault!=nil)
    {
        DDXMLElement* faultString=[CookBookHelper getNode:fault name:@"faultstring"];
        id faultObj=nil;
        DDXMLElement* faultDetail=[CookBookHelper getNode:fault name:@"detail"];
        if(faultDetail!=nil)
        {
        ...
        OutputFault=[[CookBookSoapError alloc] initWithDetails:[faultString stringValue] details:faultObj];
    }
}
...
-(void) sendImplementation:(NSMutableURLRequest*) request callbackDelegate:(CookBookCLB) callbackDelegate
{
    if(self.EnableLogging) {
        NSString* strRequest = [[NSString alloc] initWithData: request.HTTPBody encoding: NSUTF8StringEncoding];
        NSLog(@"%@", strRequest);
