...
{
    DDXMLNode* child=[DicHelper getAttribute:node name:name];
    return child!=nil;
}

+(BOOL) hasValue:(DDXMLElement*) node name:(NSString*)name
{
    DDXMLElement* child=[DicHelper getNode:node name:name];
    if(child!=nil)
    {
        DDXMLNode* nilAttr=[DicHelper getAttribute:child name:@"nil" url:XSI];
        return nilAttr==nil;
    }
    return NO;
}
