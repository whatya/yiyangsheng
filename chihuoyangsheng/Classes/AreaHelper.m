...
{
    DDXMLNode* child=[AreaHelper getAttribute:node name:name];
    return child!=nil;
}

+(BOOL) hasValue:(DDXMLElement*) node name:(NSString*)name
{
    DDXMLElement* child=[AreaHelper getNode:node name:name];
    if(child!=nil)
    {
        DDXMLNode* nilAttr=[AreaHelper getAttribute:child name:@"nil" url:XSI];
        return nilAttr==nil;
    }
    return NO;
}
