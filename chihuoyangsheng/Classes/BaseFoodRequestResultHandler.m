...
#import "BaseFoodSoapError.h"



@implementation BaseFoodRequestResultHandler
...
if ((self=[super init])) {
        ...
        if(!classNames)
        {
            ...
            [BaseFoodgetById class],@"http://service.food.app/^^getById",
            [BaseFoodgetByIdResponse class],@"http://service.food.app/^^getByIdResponse",
            [BaseFoodbaseFood class],@"http://service.food.app/^^baseFood",
            [BaseFoodfindBaseFood class],@"http://service.food.app/^^findBaseFood",
            [BaseFoodpageParam class],@"http://service.food.app/^^pageParam",
            [BaseFoodfindBaseFoodResponse class],@"http://service.food.app/^^findBaseFoodResponse",
            [BaseFoodjsonData class],@"http://service.food.app/^^jsonData",
            nil]; 
...
{
    DDXMLNode* refAttr=[BaseFoodHelper getAttribute:node name:@"Ref" url:MS_SERIALIZATION_NS];
    if(refAttr!=nil)
    {
        return [referencesTable objectForKey:[refAttr stringValue]];
    }
    
    DDXMLNode* nilAttr=[BaseFoodHelper getAttribute:node name:@"nil" url:XSI];
    if(nilAttr!=nil && [[nilAttr stringValue]boolValue])
    {
        return nil;
    }

    DDXMLNode* typeAttr=[BaseFoodHelper getAttribute:node name:@"type" url:XSI];
    if(typeAttr !=nil)
    {
    ...
    DDXMLNode* hrefAttr=[BaseFoodHelper getAttribute:node name:@"href" url:@""];
    if(hrefAttr==nil)
    {
        hrefAttr=[BaseFoodHelper getAttribute:node name:@"ref" url:@""];
    }
    if(hrefAttr!=nil)
    {
    ...
    DDXMLNode* idAttr=[BaseFoodHelper getAttribute:node name:@"Id" url:MS_SERIALIZATION_NS];
    if(idAttr!=nil)
    {
...
-(id) createInstance:(Class) type node: (DDXMLNode*)node request :(BaseFoodRequestResultHandler *)request
{
    SEL initSelector=@selector(initWithXml:__request:);
    id allocObj=[type alloc];
    IMP imp = [allocObj methodForSelector:initSelector];
    id (*func)(id, SEL, DDXMLNode*, BaseFoodRequestResultHandler *) = (void *)imp;
...
-(DDXMLElement*) writeElement:(id)obj type:(Class)type name: (NSString*)name URI: (NSString*) URI parent:(DDXMLElement*) parent skipNullProperty:(BOOL)skipNullProperty
{
    ...
    if([obj conformsToProtocol:@protocol(BaseFoodIReferenceObject)])
    {
...
-(void)setResponse:(NSData *)response
{
    ...
    OutputBody=[BaseFoodHelper getNode:__root  name:@"Body" URI:@"http://schemas.xmlsoap.org/soap/envelope/"];
    OutputHeader=[BaseFoodHelper getNode:__root  name:@"Header" URI:@"http://schemas.xmlsoap.org/soap/envelope/"];
    DDXMLElement* fault=[BaseFoodHelper getNode:OutputBody  name:@"Fault" URI:@"http://schemas.xmlsoap.org/soap/envelope/"];
    if(fault!=nil)
    {
        DDXMLElement* faultString=[BaseFoodHelper getNode:fault name:@"faultstring"];
        id faultObj=nil;
        DDXMLElement* faultDetail=[BaseFoodHelper getNode:fault name:@"detail"];
        if(faultDetail!=nil)
        {
        ...
        OutputFault=[[BaseFoodSoapError alloc] initWithDetails:[faultString stringValue] details:faultObj];
    }
}
...
-(void) sendImplementation:(NSMutableURLRequest*) request callbackDelegate:(BaseFoodCLB) callbackDelegate
{
    if(self.EnableLogging) {
        NSString* strRequest = [[NSString alloc] initWithData: request.HTTPBody encoding: NSUTF8StringEncoding];
        NSLog(@"%@", strRequest);
