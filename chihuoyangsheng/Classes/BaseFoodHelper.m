...
{
    DDXMLNode* child=[BaseFoodHelper getAttribute:node name:name];
    return child!=nil;
}

+(BOOL) hasValue:(DDXMLElement*) node name:(NSString*)name
{
    DDXMLElement* child=[BaseFoodHelper getNode:node name:name];
    if(child!=nil)
    {
        DDXMLNode* nilAttr=[BaseFoodHelper getAttribute:child name:@"nil" url:XSI];
        return nilAttr==nil;
    }
    return NO;
}
