...
#import "AreagetCityByIdResponse.h"
#import "Areacity.h"
#import "AreafindCity.h"
#import "AreagetProvinceById.h"
...
@implementation AreaRequestResultHandler
...
if ((self=[super init])) {
        ...
        if(!classNames)
        {
            ...
            [AreafindProvince class],@"http://service.area.app/^^findProvince",
            [Areaprovince class],@"http://service.area.app/^^province",
            [AreagetAllProvince class],@"http://service.area.app/^^getAllProvince",
            [AreagetCityById class],@"http://service.area.app/^^getCityById",
            [AreagetCityByIdResponse class],@"http://service.area.app/^^getCityByIdResponse",
            [Areacity class],@"http://service.area.app/^^city",
            [AreafindCity class],@"http://service.area.app/^^findCity",
            [AreagetProvinceById class],@"http://service.area.app/^^getProvinceById",
            [AreagetProvinceByIdResponse class],@"http://service.area.app/^^getProvinceByIdResponse",
            nil]; 
...
{
    DDXMLNode* refAttr=[AreaHelper getAttribute:node name:@"Ref" url:MS_SERIALIZATION_NS];
    if(refAttr!=nil)
    {
        return [referencesTable objectForKey:[refAttr stringValue]];
    }
    
    DDXMLNode* nilAttr=[AreaHelper getAttribute:node name:@"nil" url:XSI];
    if(nilAttr!=nil && [[nilAttr stringValue]boolValue])
    {
        return nil;
    }

    DDXMLNode* typeAttr=[AreaHelper getAttribute:node name:@"type" url:XSI];
    if(typeAttr !=nil)
    {
    ...
    DDXMLNode* hrefAttr=[AreaHelper getAttribute:node name:@"href" url:@""];
    if(hrefAttr==nil)
    {
        hrefAttr=[AreaHelper getAttribute:node name:@"ref" url:@""];
    }
    if(hrefAttr!=nil)
    {
    ...
    DDXMLNode* idAttr=[AreaHelper getAttribute:node name:@"Id" url:MS_SERIALIZATION_NS];
    if(idAttr!=nil)
    {
...
-(id) createInstance:(Class) type node: (DDXMLNode*)node request :(AreaRequestResultHandler *)request
{
    SEL initSelector=@selector(initWithXml:__request:);
    id allocObj=[type alloc];
    IMP imp = [allocObj methodForSelector:initSelector];
    id (*func)(id, SEL, DDXMLNode*, AreaRequestResultHandler *) = (void *)imp;
...
-(DDXMLElement*) writeElement:(id)obj type:(Class)type name: (NSString*)name URI: (NSString*) URI parent:(DDXMLElement*) parent skipNullProperty:(BOOL)skipNullProperty
{
    ...
    if([obj conformsToProtocol:@protocol(AreaIReferenceObject)])
    {
...
-(void)setResponse:(NSData *)response
{
    ...
    OutputBody=[AreaHelper getNode:__root  name:@"Body" URI:@"http://schemas.xmlsoap.org/soap/envelope/"];
    OutputHeader=[AreaHelper getNode:__root  name:@"Header" URI:@"http://schemas.xmlsoap.org/soap/envelope/"];
    DDXMLElement* fault=[AreaHelper getNode:OutputBody  name:@"Fault" URI:@"http://schemas.xmlsoap.org/soap/envelope/"];
    if(fault!=nil)
    {
        DDXMLElement* faultString=[AreaHelper getNode:fault name:@"faultstring"];
        id faultObj=nil;
        DDXMLElement* faultDetail=[AreaHelper getNode:fault name:@"detail"];
        if(faultDetail!=nil)
        {
        ...
        OutputFault=[[AreaSoapError alloc] initWithDetails:[faultString stringValue] details:faultObj];
    }
}
...
-(void) sendImplementation:(NSMutableURLRequest*) request callbackDelegate:(AreaCLB) callbackDelegate
{
    if(self.EnableLogging) {
        NSString* strRequest = [[NSString alloc] initWithData: request.HTTPBody encoding: NSUTF8StringEncoding];
        NSLog(@"%@", strRequest);
